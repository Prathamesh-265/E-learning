...
export function useEnroll() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Enrolled!", description: "You can now access the course content." });
    },
    onError: (err) => {
...
export function useUpdateProgress() {
  ...
  return useMutation({
    mutationFn: async ({ id, lessonId, completed }: { id: number, lessonId: number, completed: boolean }) => {
